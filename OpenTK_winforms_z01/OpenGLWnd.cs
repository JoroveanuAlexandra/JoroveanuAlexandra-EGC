    ...
    public partial class OpenGLWnd : Form
    {
        ...
        private void LoadTexture(string imagePath)
        {
            if (File.Exists(imagePath))
            {
                ...
                // Setează parametrii de textură (opțional, în funcție de necesități)
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            // Setare culoare fundal control GL.
            textureBitmap = new Bitmap("D:\\Facultate\\Anul_3\\Semestrul_1\\EGC\\OpenTK_console_sample01\\paper.jpg.jpg"); // specifică calea către fișierul texturii
            ...
            // Setează parametrii texturii (minification, magnification, wrapping)
            GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMinFilter, (int)TextureMinFilter.Linear);
            GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMagFilter, (int)TextureMagFilter.Linear);
        ...
        private void DrawObject()
        {
            ...
            for (int i = 0; i < 35; i = i + 3)
            {
                ...
                GL.TexCoord2(0, 0); GL.Vertex3(objVertices[0, i], objVertices[1, i], objVertices[2, i]);
                GL.TexCoord2(1, 0); GL.Vertex3(objVertices[0, i + 1], objVertices[1, i + 1], objVertices[2, i + 1]);
                GL.TexCoord2(0.5f, 1); GL.Vertex3(objVertices[0, i + 2], objVertices[1, i + 2], objVertices[2, i + 2]);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                openFileDialog.Filter = "Imagini|*.bmp;*.jpg;*.jpeg;*.png;*.gif";
                openFileDialog.Title = "Selectează o imagine pentru textură";

                if (openFileDialog.ShowDialog() == DialogResult.OK)
                {
                    UnloadResources(); // Eliberează textura veche, dacă există
                    LoadTexture(openFileDialog.FileName); // Încarcă textura nouă
                    glControlDemo.Invalidate(); // Forțează redesenarea pentru a reflecta schimbarea texturii
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Eroare la încărcarea texturii: {ex.Message}", "Eroare", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}
